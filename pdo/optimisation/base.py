...
class Optimiser:
    def __init__(self, problem: PoissonControlProblem, q0: Variable,
                 c: float = 1e-4, tau: float = 0.5, trial: float | None = None,
                 tol: float = 1e-8, max_iter: int = 100,
                 min_alpha: float = 1e-14, max_backtracks: int = 60) -> None:
    ...
    def _line_search(self, control: Variable, grad: Variable, direction: Variable) -> float:
        alpha = self.trial
        ...
        for _ in range(self.max_backtracks):
            if self.problem.J(control + direction * alpha) <= J0 + self.c * alpha * slope:
                return alpha
