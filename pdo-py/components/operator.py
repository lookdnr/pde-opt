...
class PoissonOperator:
    ...
    def _assemble(self, domain: Domain2D) -> None:
        ...
        self._A = (sp.kron(Iy, Tx, format="csr") / hx**2) + (
            sp.kron(Ty, Ix, format="csr") / hy**2
        )

    # build the intermediate components of the discrete operator assembly
    def _build_components(self, N: int) -> Tuple[sp.spmatrix, sp.spmatrix]:
        ...
        T = cast(
            ...
            sp.diags(
                ...
                offsets=cast(Any, [-1, 0, 1]),
                format="csr",
            ),
        )

        # construct identity
        I = cast(sp.spmatrix, sp.eye(N, format="csr"))
    ...
    def show(self) -> None:
        ...
        plt.spy(cast(Any, self._A), markersize=0.1)
