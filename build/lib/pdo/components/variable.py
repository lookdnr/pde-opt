...
class Variable:
    ...
    def __post_init__(self):
        ...
        if self.init is None:
            ...
            ).flatten()  # init with zeros if data not provided
        else:
            arr = np.asarray(self.init, dtype=float)  # cast to array
            if (len(arr.shape) == 1) and (
                arr.shape[0] != self._shape[0] * self._shape[1]
            ):  # handle flat init data
                raise ValueError(
                    f"Expected shape {np.prod(self._shape)}, got {arr.shape[0]}"
                )
            elif (len(arr.shape) == 2) and (arr.shape != self._shape):
                raise ValueError(
                    f"Expected shape {self._shape}, got {arr.shape}"
                )  # throw error for shape mismatch
